...
do
    ...
    if [ "$ext" == "zip" ]; then
        ...
        7z e -y -o/tmp -r $tmp
        7z e -y -r -o$dir ${tmp::-3} *.otf *.ttf README* readme* LICEN* licen*
        rm -f ${tmp::-3}
    fi
    rm -f $tmp
done < font.list
...
wget "http://download.microsoft.com/download/E/6/7/E675FFFC-2A6D-4AB0-B3EB-27C9F8C8F696/PowerPointViewer.exe" -O /tmp/ppt-view.exe
mkdir -p VistaFonts
pushd /tmp
7z x -y ppt-view.exe ppviewer.cab
popd
7z e -y -oVistaFonts /tmp/ppviewer.cab *.ttc *.TTC *.ttf *.TTF *.OTF *.otf
pushd VistaFonts/
for f in `find -depth`; do mv -f ${f} ${f,,} ; done
